...
/**
 * A MatrixGraph is an implementation of the Graph abstract class that uses an
 * array to represent the edges.
 * 
...
public class GeneralizedMatrixGraph extends GeneralizedAbstractGraph {
   ...
   */
  public GeneralizedMatrixGraph(int numV, boolean directed, int weightCount) {
    super(numV, directed, weightCount);
    edges = new double[numV][][];
    if (!directed) {
      for (int i = 0; i != numV; ++i) {
        edges[i] = new double[numV][weightCount];
      ...
      for (int i = 0; i != numV; ++i) {
        edges[i] = new double[i + 1][weightCount];
        for (int j = 0; j != i + 1; ++j) {
            for(int k = 0; k < weightNum; ++k)
                edges[i][j][k] = Double.POSITIVE_INFINITY;
        }
   ...
   * weight of POSITIVE_INFINITY is returned.
   * 
   ...
   * @param source - The source vertix
   * @param dest   - The destination vertix
   * @return The weight of this edge or POSITIVE_INFINITY if no edge exists
   */
  ...
  // Iter class
  /**
   * An iterator to the edges. An Edge iterator is similar to an Iterator except
   * that its next method will always return an edge
   */
  private class Iter implements Iterator<GeneralizedEdge> {
     ...
     * 
     * @throws NoSuchElementException - there are no more edges
    ...
    private void advanceIndex() {
      do {
        index++;
      } while (index != getNumV() && Double.POSITIVE_INFINITY == getEdgeValue(source, index)[0]);
    }
  }
}
