...
public class OnlineDepot implements Serializable{
    private ArrayContainer<Depot> depots;

    /**
     * OnlineDepot constructor.
     */
    public OnlineDepot(){
        depots = new ArrayContainer<>();
    }
    ...
    public void removeDepot(Depot depot){
        for(int i = 0; i < depots.getSize(); ++i){
    ...
    public boolean removeItemFromDepots(Order order){
        ...
        for(int i = 0; i < depots.getSize() && found == false; ++i){
    ...
    public String searchProduct(String name){
        ...
        for(int i = 0; i < depots.getSize(); ++i){
            ArrayContainer<Item> founded = depots.get(i).searchProduct(name);
            if(founded.getSize() > 0)
                strBuild.append(depots.get(i).getBranch().getName() + ":\n");
            for(int j = 0; j < founded.getSize(); ++j){
                strBuild.append("   ").append(founded.get(j)).append("\n");
            }
        }
